...
plot_TE_condition_zoom <- ggplot(TE_condition, aes(x= ribo.mRNA_ratio_mock, y=ribo.mRNA_ratio_infect6h ))+
  ...
  geom_text(data = highlight, aes(x = ribo.mRNA_ratio_mock, y = ribo.mRNA_ratio_infect6h, label = Geneid_stable), 
...
TE_filtered <- TE_counts %>% 
  ...
  filter(te.mock.1>0.01&te.mock.1<15,
         ...
         te.infect6h.4>0.01&te.infect6h.4<15,
         te.infect6h.5>0.01&te.infect6h.5<15,
         te.infect6h.6>0.01&te.infect6h.6<15) %>%
  filter(te.mock.1!="NA",te.mock.2!="NA",te.mock.3!="NA",te.infect6h.4!="NA",te.infect6h.5!="NA",te.infect6h.6!="NA" ) %>% 
  column_to_rownames("Geneid") %>% 
  as.matrix()

#creating the objects for modelling
metadata <- read.csv("data/te_metadata.csv")
...
topTable(fit, number = 7667, sort.by = "logFC", p.value = 0.05 )
...
FC_TE <- topTable(fit, number = 9838,sort.by = "logFC" ) %>% 
  as_tibble(rownames = "Geneid") %>% 
  mutate(FC = 2^(logFC)) %>%
  mutate(Geneid = str_replace_all(Geneid, c("ENSG00000177076.6"="ACER2",
                                            "ENSG00000155287.11"="SLC25A28",
                                            "ENSG00000152439.13" = "ZNF773",
                                            "ENSG00000142408.5" = "CACNG8",
                                            "ENSG00000146955.11" = "RAB19",
                                            "ENSG00000157823.17" = "AP3S2",
                                            "ENSG00000221823.11" = "PPP3R1",
...
plot_volcano_TE <- EnhancedVolcano(FC_TE,
    ...
    xlim = c(-1.5, 1.5),
    ylim = c(0, 3),
    title = "Uninfected versus infected 6hr",
    FCcutoff = 0.5,
    pCutoff = 0.05)

ggsave(filename = "results/plot_volcano_TE.png", plot = plot_volcano_TE, width = 15, height = 15, dpi = 300, units = "cm")
